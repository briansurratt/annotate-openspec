...
- [x] 3.4 Commit: `feat(cmd): add daemon subcommand`
...
- [x] 4.4 Commit: `feat(cmd): add enqueue subcommand`
...
- [x] 5.6 Commit: `feat(cmd): add apply, report, status, index rebuild stub subcommands`

## 6. M1 end-to-end verification

- [x] 6.1 Run `annotate daemon` and save a `.md` file; verify a queue row and `enqueued` event log entry exist in SQLite
- [x] 6.2 Run `go build ./cmd/annotate/` and confirm the binary compiles without errors
- [x] 6.3 Run full test suite (`go test ./...`) and confirm all tests pass
